...
# === PARAMETRI GLOBALI PER TUTTI I METODI ===
MAX_STEERING_ANGLE = 45.0  # Angolo massimo di sterzata in gradi
SLOPE_THRESHOLD_HIGH = 1.5  # Soglia per pendenze "estreme"
SLOPE_THRESHOLD_LOW = 0.2   # Soglia per pendenze "minime"

base_exp = 2 # Base per la crescita esponenziale
power_low = 0.7 # Potenza per pendenze basse
power_high = 2.5 # Potenza per pendenze alte
...
def calculate_steering_angle(slope, method='exponential'):
    ...
    if method == 'linear':
        ...
        return normalized_slope * MAX_STEERING_ANGLE # Scala all'angolo massimo di 45°
        
    elif method == 'adaptive_exponential':
        """
        Comportamento esponenziale che si adatta alla pendenza:
        - Pendenze basse: risposta dolce
        - Pendenze medie: crescita esponenziale
        - Pendenze alte: saturazione rapida
        """
        
        # Normalizza la pendenza
        abs_slope = abs(slope)
        sign = np.sign(slope)
        
        if abs_slope <= SLOPE_THRESHOLD_LOW:
            # Zona lineare per pendenze molto basse
            response = abs_slope / SLOPE_THRESHOLD_LOW * 0.3
        elif abs_slope <= SLOPE_THRESHOLD_HIGH:
            # Zona esponenziale per pendenze medie
            normalized = (abs_slope - SLOPE_THRESHOLD_LOW) / (SLOPE_THRESHOLD_HIGH - SLOPE_THRESHOLD_LOW)
            response = 0.3 + (np.exp(base_exp * normalized) - 1) / (np.exp(base_exp) - 1) * 0.7
        else:
            # Saturazione per pendenze estreme
            response = 1.0
        
        return sign * response * MAX_STEERING_ANGLE

    elif method == 'variable_power':
            """
            Usa potenze diverse basate sulla pendenza:
            - Pendenze basse: potenza < 1 (curva concava, risposta dolce)
            - Pendenze alte: potenza > 1 (curva convessa, risposta aggressiva)
            """
            
            abs_slope = abs(slope)
            sign = np.sign(slope)
            normalized = np.clip(abs_slope / 1.0, 0, 1)  # Normalizza rispetto a pendenza di riferimento
            
            if abs_slope <= 0.5:
                # Usa potenza bassa per pendenze contenute
                power = power_low
            else:
                # Interpola tra potenza bassa e alta
                transition = (abs_slope - 0.5) / 0.5  # Da 0 a 1
                power = power_low + (power_high - power_low) * transition
            
            response = normalized ** power
            return sign * response * MAX_STEERING_ANGLE

    elif method == 'tanh':
        # La tangente iperbolica è simmetrica rispetto all'origine, con risposta smooth e saturazione naturale
        k = 2.0  # Ripidità
        response = np.tanh(k * slope)
        return response * MAX_STEERING_ANGLE
        ...
        if abs_slope < 0.2:
            angle = sign * abs_slope * 30  # Più sensibile per pendenze piccole
        elif abs_slope < 1.0:
            angle = sign * (12 + (abs_slope - 0.2) * 30)  # Intervallo medio
        else:
            angle = sign * MAX_STEERING_ANGLE  # Saturazione
        
        return np.clip(angle, -MAX_STEERING_ANGLE, MAX_STEERING_ANGLE)
        ...
        return sigmoid_response * MAX_STEERING_ANGLE
        ...
        return sign * quadratic_response * MAX_STEERING_ANGLE
