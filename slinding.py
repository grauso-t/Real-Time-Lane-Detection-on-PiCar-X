...
Kp = 1.5  # Proportional gain for steering
base_speed = 0  # Set a positive value to make the robot move
...
while True:
    ...
    if detected_lines is not None:
        for seg in detected_lines:
            ...
            if abs(mid_x - center_x) < min_distance_from_center:
                print("Linea troppo centrale, mantenendo l'ultimo angolo.")
                continue  # Ignore segment and keep last angle

            if x2 - x1 == 0:
                if mid_x < bird_eye.shape[1] / 2:
                    left_segments.append((x1, y1, x2, y2))
                    print("Linea di sinistra " + str(mid_x) + " " + str(bird_eye.shape[1] / 2))
                else:
                    right_segments.append((x1, y1, x2, y2))
                    print("Linea di destra" + " " + str(mid_x) + " " + str(bird_eye.shape[1] / 2))
                continue
            ...
            if slope < 0 and mid_x < bird_eye.shape[1] / 2:
                left_segments.append((x1, y1, x2, y2))
                print("Linea di sinistra"+str(mid_x)+str(bird_eye.shape[1] / 2)+" slope:"+str(slope))
                
            elif slope > 0 and mid_x >= bird_eye.shape[1] / 2:
                right_segments.append((x1, y1, x2, y2))
                print("Linea di destra"+str(mid_x)+str(bird_eye.shape[1] / 2)+ " slope:"+str(slope))
    ...
    intended_steering_angle = -Kp * deviation
    steering_angle = max(-50.0, min(50.0, intended_steering_angle))
    both_lines_truly_detected = (L_bot_x_detected_val is not None) and (R_bot_x_detected_val is not None)

    if both_lines_truly_detected and (lane_width < 180 or lane_width > 340):
        steering_angle = last_steering_angle
        print(f"Larghezza {lane_width:.1f} non valida, mantengo angolo {last_steering_angle:.1f}")
